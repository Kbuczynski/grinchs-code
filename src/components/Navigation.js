...
const Navigation = () => {
  ...
  const onBurgerChange = event => {
    setBurgerOpen(event.isOpen);
  };

  const onLinkClicked = () => {
    if (burgerOpen) onBurgerChange({ isOpen: false });
  };
  ...
  const renderBurger = () => {
    return (
      <Menu
        id={"menu"}
        pageWrapId={"app"}
        outerContainerId={"navigation-container-burger"}
        disableAutoFocus
        isOpen={burgerOpen}
        onStateChange={e => onBurgerChange(e)}
      >
  ...
  if (windowDimensions.width > 1000) {
    ...
    return (
      <div className={"navigation-container-burger"}>{renderBurger()}</div>
    );
  }
};
